...
def closestUnit(arr1, arr2, n):
    ...
    l1 = arr1[:mid]
    r1 = arr1[mid:]
    dl, l_pair = closestUnit(l1, arr2, mid)  # 왼쪽 최근접 점의 쌍 반환
    dr, r_pair = closestUnit(r1, arr2, n - mid)  # 오른쪽 최근접 점의 쌍 반환
...
def closestPair(arr, n):
    ...
    while True:  # 가장 짧은 거리의 쌍을 모두 찾을 때까지 반복
        ...
        # 발견된 쌍은 리스트에서 삭제
        arr = [p for p in arr if
               not ((p.x == pair[0].x and p.y == pair[0].y) or (p.x == pair[1].x and p.y == pair[1].y))]
...
def main():
    # 파일 입력 받기
    arr = []
    with open("input_closest_pair.txt", "r") as f:
        while True:
            line = f.readline()
            if not line:
                break
